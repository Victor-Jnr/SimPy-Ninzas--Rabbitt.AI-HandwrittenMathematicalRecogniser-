#   -The TrainingModel class: Handles loading and reshaping the data, data augmentation, model instantiation, training (using callbacks and early stopping), visualization of training history (accuracy/loss curves), and saving the trained model along with its history.-    #
#   -A custom callback (myCallback): Monitors training and stops it once certain accuracy and loss thresholds are met.-    #
#   -Both training scripts use this file. Abstracts away common operations so that you can plug in different network architectures and configurations without duplicating the data handling and training loop code.-    #
#   -Also have edited the tkagg to Agg as we are using a Dockeer container and hence do not support a headless GUI-    #
#   --    #
#   -Summary:Victor-Jnr-    #
...
class TrainingModel:
    ...
    def adjust_data(self):
        ...
        self.images = images.reshape(images.shape[0], self.image_width, self.image_height, self.image_channels) 
    ...
    def train(self):
        ...
        callbackES1 = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=3, restore_best_weights=True) 
        callbackES2 = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True) 
        ...
        self.train = self.classifier.fit( 
            ...
            steps_per_epoch=(len(self.images) // self.configs['model']['batch_size']), 
            validation_data=(self.test_images, self.test_labels),
            callbacks=[callbacks, callbackES1, callbackES2] 
        )
        return self.train
